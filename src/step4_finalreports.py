# Process all current seizures except those excluded by the 
# seizure_modifications file, computing overall match probabilities 
# and writing files for the network drawing program.

# Assumptions:

# Any subdirectory containing "nsavannah" or "nforest" is assumed to be
# a seizure and handled accordingly, unless listed in seizure_modifications
# for exclusion.

# Familial matching has been previously run on all seizures.

# Input files:

# It is meant to be run in the parent directory of all seizurs.  WARNING:
# it will assume that any subdirectory containing "nforest" or "nsavannah"
# subdirectories is a seizure, and proceed accordingly.
...
if len(sys.argv) != 5:
  ...
  print("This program jointly processes all available seizures")
  print("Be sure familial matching has been run on all desired seizures first!")
