...
# improve the plotting.  It has two key choices:

# (1)  Node coloring based on transit port versus Louvain clustering
# (2)  Node size based on year versus number of connections

# These are set by the two constants below.
# The Nature paper used port_coloring = False and year_size = False

# port coloring:  True colors by port, False by Louvain cluster
port_coloring = True
...
def read_portcolors(name_to_num,num_to_port,colorfile):
  ...
  for name, num in name_to_num.items():
    ...
    textdict[num] = text
    sizedict[num] = year
  return filldict,textdict,sizedict
...
filldict,textdict,sizedict = read_portcolors(name_to_num,num_to_port,colorfile)

sgraph.vertex_properties["mary_size"] = sgraph.new_vertex_property("int")
sgraph.vertex_properties["mary_fill_color"] = sgraph.new_vertex_property("string")
...
for vertex in sgraph.vertices():
  ...
  sgraph.vertex_properties["mary_text_color"][vertex] = sizedict[key]

# set node color conditionally (by port or partition)
for vertex in sgraph.vertices():
  key = sgraph.vertex_properties["id"][vertex]
  if port_coloring:
    # color by transit port
    sgraph.vertex_properties["mary_fill_color"][vertex] = filldict[key]
  else:
    # color by Louvain partition
    mypart = sgraph.vertex_properties["mary_partition"][vertex]
    sgraph.vertex_properties["mary_fill_color"][vertex] = printcolors[mypart]
...
# rescale sizes
sgraph.vertex_properties["mary_size"] = prop_to_size(sgraph.vertex_properties["mary_size"],mi=20.0,ma=70.0)
...
fill_color = sgraph.vertex_properties["mary_fill_color"]
...
# draw normal graph with edges
#pos1 = graph_draw(sgraph,pos=pos,vertex_text=text,vertex_fill_color=fill_color,output_size=(1000,1000),vertex_font_size=16,vertex_font_weight=cairo.FONT_WEIGHT_BOLD,edge_pen_width=pen,vertex_text_position=-2,vertex_text_color=text_color,vertex_size=size,vertex_color="k")

pos1 = graph_draw(sgraph,pos=pos,vertex_text=text,vertex_fill_color=fill_color,output_size=(1000,1000),vertex_font_size=16,vertex_font_weight=cairo.FONT_WEIGHT_BOLD,vertex_text_position=-2,vertex_text_color=text_color,vertex_size=size,vertex_color="k")

for vertex in pos:
  print(vertex)

pos2 = graph_draw(sgraph,pos=pos1,vertex_text=text,vertex_fill_color=fill_color,output_size=(1000,1000),vertex_font_size=16,vertex_font_weight=cairo.FONT_WEIGHT_BOLD,vertex_text_position=-2,vertex_text_color=text_color,vertex_size=size,vertex_color="k")

# write graph to file
#pos2 = graph_draw(sgraph,pos=pos1,vertex_text=text,vertex_fill_color=fill_color,output_size=(1000,1000),vertex_font_size=16,vertex_font_weight=cairo.FONT_WEIGHT_BOLD,edge_pen_width=pen,vertex_text_position=-2,vertex_text_color=text_color,vertex_size=size,vertex_color="k")

# write no-lines version to file
#pos3 = graph_draw(sgraph,pos=pos,vertex_text=text,vertex_fill_color=fill_color,output_size=(1000,1000),vertex_font_size=16,vertex_font_weight=cairo.FONT_WEIGHT_BOLD,edge_pen_width=pen,vertex_text_position=-2,vertex_text_color=text_color,vertex_size=size,vertex_color="k")
