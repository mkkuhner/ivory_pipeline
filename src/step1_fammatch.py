...
def backup_archive(arch_dir, arch_name):
  if not os.path.isdir(arch_dir + arch_name):
    ...
    exit(-1)
  backupdir = arch_dir + arch_name[:-1] + "_backups/"
  # delete previous backup directory
  if os.path.isdir(backupdir):
    command = ["rm","-rf",backupdir]
    iv.run_and_report(command,"Unable to delete old backup directory" + backupdir)
  # copy archive to backup directory
  command = ["cp","-r",arch_dir + arch_name,backupdir]
  iv.run_and_report(command,"Unable to back up archive")
...
def restore_archive(arch_dir):
  ...
  if os.path.isdir(arch_dir):
    ...
    iv.run_and_report(command,"Unable to snapshot archive for debugging")

  # delete archive directory
  if os.path.isdir(arch_dir):
    command = ["rm","-rf",arch_dir]
    iv.run_and_report(command,"Unable to restore from backup " + backupdir)

  # copy backup directory into archive directory
  command = ["cp","-r",backupdir,arch_dir]
  iv.run_and_report(command,"Unable to restore archive from backups")
...
backup_archive(arch_dir, arch_name)
