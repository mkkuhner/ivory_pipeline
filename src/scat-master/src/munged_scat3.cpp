...
int main ( int argc, char** argv)
{
  ...
  // DEBUG
  cout << "Printing old map and TERMINATING" << endl;
  pair<int,int> latbounds = mymapgrid.get_latbounds();
  pair<int,int> longbounds = mymapgrid.get_longbounds();
  double minlat = (float)latbounds.first;
  double minlong = (float)longbounds.first;
  int mygridsize = mymapgrid.get_gridsize();
  double mylat, mylong;
  cout << endl;
  for(int tlat=mygridsize; tlat > 0; --tlat) {
    for(int tlong=0; tlong < mygridsize; ++tlong) {
       mylat = minlat + tlat + 0.5;
       mylong = minlong + tlong + 0.5;
       bool gridinrange = mymapgrid.in_range(mylat,mylong);
       mylat = PI * mylat / 180.0;
       mylong = PI * mylong / 180.0;
       bool isinrange = InElephantRange(mylong,mylat);
       if (isinrange && gridinrange) cout << "*";
       if (isinrange && !gridinrange) cout << "1";
       if (!isinrange && gridinrange) cout << "2";
       if (!isinrange && !gridinrange) cout << "-";
    }
    cout << endl;
  }
